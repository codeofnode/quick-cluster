{{/*
Expand the name of the chart.
*/}}
{{- define "chart.name" -}}
{{- default .Chart.Name .Values.nameOverride | trunc 63 | trimSuffix "-" }}
{{- end }}

{{/*
Create a default fully qualified app name.
We truncate at 63 chars because some Kubernetes name fields are limited to this (by the DNS naming spec).
If release name contains chart name it will be used as a full name.
*/}}
{{- define "chart.fullname" -}}
{{- .Values.nameOverride | trunc 63 | trimSuffix "-" }}
{{- end }}

{{/*
Create chart name and version as used by the chart label.
*/}}
{{- define "chart.chart" -}}
{{- printf "%s-%s" .Chart.Name .Chart.Version | replace "+" "_" | trunc 63 | trimSuffix "-" }}
{{- end }}

{{/*
Common labels
*/}}
{{- define "chart.labels" -}}
helm.sh/chart: {{ include "chart.chart" . }}
{{ include "chart.selectorLabels" . }}
{{- if .Chart.AppVersion }}
app.kubernetes.io/version: {{ .Chart.AppVersion | quote }}
{{- end }}
app.kubernetes.io/managed-by: {{ .Release.Service }}
{{- end }}

{{/*
Selector labels
*/}}
{{- define "chart.selectorLabels" -}}
app.kubernetes.io/name: {{ include "chart.name" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
{{- end }}
{{- define "chart.myappLabels" -}}
myapp/kind: {{ .Values.myapp.kind }}
myapp/type: {{ .Values.myapp.type }}
{{- end }}

{{/*
Create the name of the service account to use
*/}}
{{- define "chart.serviceAccountName" -}}
{{- if .Values.serviceAccount.create }}
{{- default (include "chart.fullname" .) .Values.serviceAccount.name }}
{{- else }}
{{- default "default" .Values.serviceAccount.name }}
{{- end }}
{{- end }}


{{/*
Create the command to start container with
*/}}
{{- define "chart.startingCommand" -}}
{{- if $.Values.debugMode }}
- "sleep"
- "1000000"
{{- else }}
{{- $clientCommand := "while true; do allrounder -i -d $ requests.json; sleep 30; done" }}
{{- $serverCommand := "/usr/local/lib/node_modules/allrounder/node_modules/json2server/bin/json2server" }}
{{- if eq .Values.myapp.kind "client" }}
- sh
- -c
- "{{- printf "%s" $clientCommand }}"
{{- else if eq .Values.myapp.kind "server" }}
- "{{- printf "%s" $serverCommand }}"
{{- else }}
- sh
- -c
- "{{- printf "%s & %s" $serverCommand $clientCommand }}"
{{- end }}
{{- end }}
{{- end }}
